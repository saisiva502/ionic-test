...
var ResListPageRoutingModule = /** @class */ (function () {
    function ResListPageRoutingModule() {
    }
    ...
    return ResListPageRoutingModule;
}());
...
var ResListPageModule = /** @class */ (function () {
    function ResListPageModule() {
    }
...
var ResListPage = /** @class */ (function () {
    function ResListPage(datePicker, router, location, _resService, activatedRoute) {
    ...
    ResListPage.prototype.ngOnInit = function () {
    };
    ResListPage.prototype.ionViewDidEnter = function () {
        this.type = this.activatedRoute.snapshot.params['type'];
        this.getAllIntilaData();
    };
    ResListPage.prototype.getAllIntilaData = function () {
        var _this = this;
        this._resService.getAllResData(this.type, this.intial_list).then(function (res) {
            ...
            if (res.status === "completed") {
                _this.list_complete = "true";
                console.log("completed", _this.list_complete);
            }
        });
    };
    ResListPage.prototype.clearFilter = function () {
        this.from_date = '';
        this.to_date = "";
        this.list_data = this.temp_data;
    };
    ResListPage.prototype.applyFilter = function () {
        var _this = this;
        if (this.from_date && this.to_date) {
            ...
            this.list_data.map(function (item, index) {
                if (from_dateParse <= Date.parse(item.date) && to_dateParse >= Date.parse(item.date)) {
                    // alert(item + "djdjdjjd")
                    data.push(item);
                }
                if (index = _this.list_data.length - 1) {
                    _this.list_data = list_filter_data;
                }
            });
            this.list_data = data;
    ...
    ResListPage.prototype.getFromDatePicker = function () {
        var _this = this;
        this.datePicker.show({
            ...
            if (day.length < 2)
                day = '0' + day;
            _this.from_date = [year, month, day].join('-');
        }, function (err) { return alert('Error occurred while getting date: ' + err); });
    };
    ResListPage.prototype.getToDatePicker = function () {
        var _this = this;
        this.datePicker.show({
            date: new Date(),
            mode: 'date',
            ...
            if (day.length < 2)
                day = '0' + day;
            _this.to_date = [year, month, day].join('-');
        }, function (err) { return alert('Error occurred while getting date: ' + err); });
    };
    ResListPage.prototype.getFutureRes = function (item) {
        console.log(item);
        this.router.navigate(['res-detail', "future", item.id]);
    };
    ResListPage.prototype.goBack = function () {
        this.location.back();
    };
    ResListPage.prototype.doReorder = function (ev) {
    ...
    ResListPage.prototype.loadData = function (event) {
        console.log("Success");
    ...
    ResListPage.ctorParameters = function () { return [
...
//# sourceMappingURL=res-list-res-list-module-es5.js.map